...
export const transformCustomersForDelivery = (
  ...
  const activeRelations = relations.filter(
    (rel) => rel.thruDate === null && rel.productId !== null,
  );

  return sortedCustomers.map((item) => {
    const customerRelations = activeRelations.filter(
      (rel) => rel.customerId === item.customer.customerId,
    );

    const associatedProductIds = customerRelations
      .map((rel) => rel.productId!)
      .filter((id) => id !== null);

    const associatedProductPrices: Record<
      number,
    ...
    return {
      ...
      address:
        `${item.customer.address1}${item.customer.address2 ? ", " + item.customer.address2 : ""}, ${item.customer.city || ""} ${item.customer.pincode || ""}`.trim(),
      deliveredItems: [],
      paymentReceived: 0,
